...
export async function getCart(): Promise<Product[]> {
  ...
  return (data ?? [])
    .filter((row): row is typeof row & { products: NonNullable<(typeof row)['products']> } =>
      Boolean(row.products),
    )
    .map((row) => ({ ...productFromRow(row.products as any), addedAt: row.added_at }));
}
...
/** Open product URLs in new tabs. Uses the Lani extension when present (popup blockers kill multi-open from the page). */
export function openProductTabs(urls: string[]): { via: 'extension' | 'window' } {
  const list = urls.filter((u) => typeof u === 'string' && u.length > 0);
  if (!list.length) return { via: 'window' };

  if (
    typeof window !== 'undefined' &&
    (window as Window & { __LANI_EXTENSION__?: boolean }).__LANI_EXTENSION__
  ) {
    window.dispatchEvent(new CustomEvent('lani:open-tabs', { detail: { urls: list } }));
    return { via: 'extension' };
  }
